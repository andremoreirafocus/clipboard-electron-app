...
const createWindow = () => {
  ...
  if (!createClippingShortcut) {
    console.error('Get clipping registration Failed', 'create-clipping');
  }
  const createWriteClippingShortcut = globalShortcut.register(
    'CommandOrControl+@',
    () => {
      console.log('Going to send a message to write a clipping');
      mainWindow.webContents.send('create-write-clipping');
    },
  );
  if (!createWriteClippingShortcut) {
    console.error('Write clipping Registration Failed', 'create-write-clipping');
  }
